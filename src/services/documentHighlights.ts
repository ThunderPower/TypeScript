...
namespace ts.DocumentHighlights {
    ...
    function getHighlightSpans(node: Node, sourceFile: SourceFile): HighlightSpan[] {
        ...
        function useParent<T extends Node>(node: Node, test: (node: Node) => node is T, cb: (t: T, sf: SourceFile) => Node[] | undefined): HighlightSpan[] | undefined {
            return test(node) ? highlightSpans(cb(node, sourceFile)) : undefined;
        }
